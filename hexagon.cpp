...
static void init (void)
{
    ...
    glClearColor (1.0, 1.0, 1.0, 0.0);  //Dipsplay-window color = white..
    /*Set up a display list for a red regular hexagon, */
    regHex = glGenLists (1); //Get an identifier for the dispaly list
    glNewList (regHex, GL_COMPILE);
    glColor3f(1.0, 0.0, 0.0); //set the fill color for hexagon to red
    glBegin (GL_POLYGON);
        for (k = 0; k < 6; k++) {
            theta = TWO_PI * k / 6.0;
